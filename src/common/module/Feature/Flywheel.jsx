  ...
  const shapes = pie.map((slice, i) => {
    ...
    return (
      ...
      <g
        ...
        style={{ opacity: i === activeIndex ? 1 : 0.25 }} // Keep this for active/inactive logic
      >
        <path d={slicePath} fill={colors[i % colors.length]} />
        <text x={labelPosX} y={inflexionPoint[1]} textAnchor={textAnchor} dominantBaseline="middle" fontSize="16" fill="black">
          {label}
        </text>
      </g>
    );
